...
{
  const auto sorted = QuickSort(std::vector<int>{});
...
{
  const auto sorted = QuickSort(std::vector<int>{8, 7});
...
{
  const auto sorted = QuickSort(std::vector<int>{5, 0, 1, 8, 7});
...
TEST_CASE("quicksort-sorted", "[quicksort]")
{
  ...
  const auto sorted = QuickSort(expected);
  CHECK(sorted == expected);
}
...
TEST_CASE("quicksort-100", "[quicksort]")
{
  const auto sorted = QuickSort(std::vector<int>{
          ...
          47,	97,	92,	6,	20, 74,	4,	84,	30,	94,
          16,	95,	73,	5,	90, 19,	2,	97,	73,	17,
          27,	34,	94,	54,	80, 65,	43,	82,	1,	55,
          60,	78,	96,	43,	80, 36,	39,	54,	76,	84,
          74,	55,	14,	66,	80, 3,	3,	55,	97,	49,
          98,	71,	40,	27,	55, 3,	99,	58,	42,	48,
          18,	47,	28,	5,	13, 12,	17,	29,	52,	96
      });
  const auto expected = std::vector<int>{
          1, 1, 2, 3, 3, 3, 4, 4, 4, 5,
          5, 6, 6, 6, 12, 12, 12, 13, 14, 14,
          14, 14, 16, 17, 17, 18, 19, 19, 20, 25,
          27, 27, 28, 29, 30, 32, 34, 34, 36, 39,
          40, 42, 42, 42, 43, 43, 47, 47, 47, 47,
          47, 48, 48, 49, 52, 52, 54, 54, 55, 55,
          55, 55, 55, 57, 58, 60, 61, 61, 65, 66,
          70, 71, 73, 73, 74, 74, 76, 78, 80, 80,
          80, 82, 84, 84, 85, 90, 90, 92, 94, 94,
