...
namespace euphoria::engine
{
  ...
  public:
    ...
    void
    CreateComponent(const ObjectCreationArgs& args, core::EntityId ent) override
    {
  ...
  public:
    ...
    Create(
        const game::Sprite& sprite, render::TextureCache* cache, Components* components)
    {
  ...
  ObjectCreator::FindTemplate(const std::string& name)
  {
    ...
    if(result == templates.end())
    {
      LOG_ERROR(
          ...
          << name
          << ", could be "
          << core::StringMerger::EnglishOr().Generate(core::Keys(templates))
          << ".");
      return nullptr;
    }
