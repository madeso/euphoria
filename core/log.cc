...
bool Logger::IsEnabledForLevel(LogLevel level) const
{
  return level_ <= level;
}
...
void Logger::AddLog(LogLevel level, const std::string& message)
{
  ...
  (level>=LogLevel::Warning?std::cerr:std::cout)
    << "[" << name_ << " " << LOG_LEVEL_NAMES[static_cast<int>(level)] << "] " << message << "\n";
}

namespace // local
{
  std::map<std::string, std::shared_ptr<Logger>>& Storage()
  {
...
LoggerPtr Logger::GetLogger(const std::string& name)
{
  ...
  if(result != storage.end()) return result->second;

  const auto parent_category = StripLastString(name, '.');
  auto parent = name.empty()?nullptr:GetLogger(parent_category == name ? parent_category : "").get();
