...
namespace euphoria::core
{
    namespace
    {
        ...
        double
        dot(Grad g, double x, double y)
        {
            return g.x * x + g.y * y;
        }

        double
        dot(Grad g, double x, double y, double z)
        {
            return g.x * x + g.y * y + g.z * z;
        }

        double
        dot(Grad g, double x, double y, double z, double w)
        {
    ...
    noise(double x, double y, double z, double w)
    {
        ...
        double x1 = x0 - i1
                    + G4;  // Offsets for second corner in (x,y,z,w) coords
        double y1 = y0 - j1 + G4;
        double z1 = z0 - k1 + G4;
        double w1 = w0 - l1 + G4;
        double x2 = x0 - i2
                    + 2.0 * G4;  // Offsets for third corner in (x,y,z,w) coords
        double y2 = y0 - j2 + 2.0 * G4;
        double z2 = z0 - k2 + 2.0 * G4;
        double w2 = w0 - l2 + 2.0 * G4;
        double x3
                = x0 - i3
                  + 3.0 * G4;  // Offsets for fourth corner in (x,y,z,w) coords
        double y3 = y0 - j3 + 3.0 * G4;
        double z3 = z0 - k3 + 3.0 * G4;
        double w3 = w0 - l3 + 3.0 * G4;
        double x4 = x0 - 1.0
                    + 4.0 * G4;  // Offsets for last corner in (x,y,z,w) coords
