...
public:
  static mat3<T> FromColMajor(T t00, T t01, T t02,
                              T t10, T t11, T t12,
                              T t20, T t21, T t22
                              ) {
    return mat3<T>(t00, t01, t02,
                   t10, t11, t12,
                   t20, t21, t22
                   );
  }
  static mat3<T> FromRowMajor(T t00, T t10, T t20,
                              T t01, T t11, T t21,
                              T t02, T t12, T t22
                              ) {
    return mat3<T>(t00, t01, t02,
  ...
  static mat3<T> FromMajor(const vec3<T>& major) {
    const T zero = 0;
    return FromRowMajor(
      major.x, zero, zero,
      zero, major.y, zero,
  ...
  static mat3<T> FromScale(const vec3<T>& scale) {
    return FromMajor(scale);
  }

  static mat3<T> FromScalar(T scalar) {
    const T z = 0;
    return FromRowMajor(
      scalar, z, z,
      z, scalar, z,
  ...
  static mat3<T> FromRotX(const Angle& a) {
    const auto c = Cos(a);
    const auto s = Sin(a);
    return FromRowMajor(
      1,  0,  0,
  ...
  static mat3<T> FromRotY(const Angle& a) {
    const auto c = Cos(a);
    const auto s = Sin(a);
    return FromRowMajor(
      c,  0,   s,
  ...
  static mat3<T> FromRotZ(const Angle& a) {
    const auto c = Cos(a);
    const auto s = Sin(a);
    return FromRowMajor(
      c,  -s,   0,
  ...
  static mat3<T> FromAxisAngle(const AxisAngle aa) {
...
#undef w
  ...
  void operator+=(const mat3<T> rhs) {
...
#undef OP
  ...
  mat3<T> Scale(const vec3<T>& scale) const {
  ...
  vec3<T> GetColumn(int c) const {
    return vec3<T>(&data[c*3]);
  }

  vec3<T> GetRow(int r) const {
    const mat3<T>& self = *this;
...
template <typename T>
std::ostream& operator<<(std::ostream& stream, const mat3<T>& m)
{
  return stream << "(" << m.GetRow(0) << ", " << m.GetRow(1) << ", " << m.GetRow(2) << ")";
}
