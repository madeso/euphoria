...
enum class LogLevel {
  Trace, Debug, Info, Warning, Error, Fatal, MAX_VALUE
};
...
public:
  ...
  bool IsEnabledForLevel(LogLevel level) const;
...
#define LOG_BASE(LOGGER, MESSAGE, LEVEL) do { \
  if(LOGGER->IsEnabledForLevel(LEVEL)) { \
    std::ostringstream log_message_stream; \
    log_message_stream << MESSAGE; LOGGER->AddLog(LEVEL, log_message_stream.str()); \
  } } while(false)

#define LOG_SPECIFY_DEFAULT_LOGGER(NAME) namespace { \
...
#define LOG_TRACE(MESSAGE) LOG_BASE(GetDefaultLogger(), MESSAGE, LogLevel::Trace)
#define LOG_DEBUG(MESSAGE) LOG_BASE(GetDefaultLogger(), MESSAGE, LogLevel::Debug)
#define LOG_INFO(MESSAGE)  LOG_BASE(GetDefaultLogger(), MESSAGE, LogLevel::Info)
#define LOG_WARN(MESSAGE)  LOG_BASE(GetDefaultLogger(), MESSAGE, LogLevel::Warning)
#define LOG_ERROR(MESSAGE) LOG_BASE(GetDefaultLogger(), MESSAGE, LogLevel::Error)
#define LOG_FATAL(MESSAGE) LOG_BASE(GetDefaultLogger(), MESSAGE, LogLevel::Fatal)
