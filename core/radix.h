...
struct Bucket10base {
  ...
  static bool KeepLooping(Int max_value, int exp) {
    return max_value/exp > 0;
  }

  static int NextValue(Int, int exp) {
    return exp * 10;
  }
};
...
void RadixSort(std::vector<T>* arr)
{
  ...
  for (int exp = Bucket::GetStart(); Bucket::KeepLooping(max_value, exp); exp = Bucket::NextValue(max_value, exp)) {
