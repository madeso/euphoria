...
#define FUN(NAME, FUNC) NAME,
  ...
  // Sine wave easing; sin(p * PI/2)
  FUN(SineIn, SineEaseIn) FUN(SineOut, SineEaseOut) FUN(Sine, SineEaseInOut)
  ...
  // Overshooting cubic easing;
  FUN(BackIn, BackEaseIn) FUN(BackOut, BackEaseOut) FUN(Back, BackEaseInOut)
...
#undef FUN
 ...
 private:
  ...
  {
    InterpolationData(const TType& t)
        : target(t)
    {
    }
