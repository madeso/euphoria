...
namespace euphoria::gui
{
      ...
      const std::map<std::string, Skin*>& skins)
  {
    ...
    if(w.button)
    {
      ...
      {
        std::shared_ptr<render::ScalableSprite> sp(new render::ScalableSprite(
            fs, skin->button_image, core::Sizef::FromSquare(2.0f), cache));
        b->SetSprite(sp);
      }
      ret.reset(b);
  ...
  #define FUN(NAME, FUNC)              \
    case ::gui::InterpolationType::NAME: \
      return core::InterpolationType::NAME;
  ...
  #undef FUN
      default:
        return core::InterpolationType::INVALID;
    }
  }
