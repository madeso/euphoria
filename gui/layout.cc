...
TableLayout::TableLayout(const std::vector<bool> expandable_rows, const std::vector<bool> expandable_cols, float combined_padding)
    : expandable_rows_(expandable_rows), expandable_cols_(expandable_cols), combined_padding_(combined_padding) {}

Sizef TableLayout::CalculateMinimumArea(const std::vector<std::shared_ptr<Widget>>& widgets) const {
  ...
  return Sizef::FromWidthHeight(std::accumulate(width.begin(), width.end(), 0), std::accumulate(height.begin(), height.end(), 0));
}

void TableLayout::DoLayout(std::vector<std::shared_ptr<Widget>>* widgets, const Rectf &area) const {
  // todo: include padding
  std::vector<float> width(expandable_cols_.size(), 0);
  std::vector<float> height(expandable_rows_.size(), 0);
  ...
  for (auto w: *widgets) {
    ...
    w->SetRect(Rectf::FromTopLeftWidthHeight(y, x, width[d.GetColumn()], height[d.GetRow()]));
  }
}

SingleRowLayout::SingleRowLayout(float padding) : padding_(padding) {}

Sizef SingleRowLayout::CalculateMinimumArea(const std::vector<std::shared_ptr<Widget>> &widgets) const {
  float width = 0;
  float height = 0;
  bool first = false;
...
void SingleRowLayout::DoLayout(std::vector<std::shared_ptr<Widget>> *widgets, const Rectf &area) const {
  float x = area.GetX();
  for (auto w: *widgets) {
    const auto& s = w->GetPreferredSize();
    w->SetRect( Rectf::FromTopLeftWidthHeight(area.GetY(), x, s.GetWidth(), s.GetHeight()) );
