...
void Node::Update(float dt, float current_time)
  {
  }
  ...
  void
  MidiInNode::DebugCallback(double dt, const std::vector<unsigned char>& bytes)
  {
    ...
    for(auto b : bytes)
    {
      ...
      if(is_status)
      {
        ...
        LOG_INFO("STAT (" << static_cast<unsigned int>(channel) << ") "
  ...
  MidiInNode::Callback(double dt, const std::vector<unsigned char>& bytes)
  {
    ...
    switch(event)
    {
      ...
      default:
        LOG_ERROR("todo: handle " << MidiEventToString(event));
        return;
    }
  }
  ...
  void
  KeyboardInputNode::OnChord(int base, bool was_pressed, float time, int first, int second)
  {
