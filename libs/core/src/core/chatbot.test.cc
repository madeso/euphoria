...
TEST_CASE("chatbot-matches-input", "[chatbot]")
{
    ...
    SECTION("match start")
    {
        ...
        CHECK(IndexOfMatchedInput(sentence, Input {start_animal, where}) == 0);
        CHECK(IndexOfMatchedInput(sentence, Input {middle_animal, where})
              == -1);
    ...
    {
        const auto where = Input::IN_MIDDLE;
        CHECK(RemoveFrom(sentence, Input {start_animal, where})
              == without_start_animal);
        CHECK(RemoveFrom(sentence, Input {middle_animal, where})
              == without_middle_animal);
        CHECK(RemoveFrom(sentence, Input {end_animal, where})
              == without_end_animal);
        CHECK(RemoveFrom(sentence, Input {full_sentence, where})
              == without_any_animals);
