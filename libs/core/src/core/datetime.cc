...
TimetWrapper
TimetWrapper::FromLocalTime(const StructTmWrapper& dt)
{
...
TimetWrapper
TimetWrapper::FromGmt(const StructTmWrapper& dt)
{
...
double
TimetWrapper::Difference(const TimetWrapper& start, const TimetWrapper& end)
{
...
StructTmWrapper
TimetWrapper::ToLocalTime() const
{
...
StructTmWrapper::StructTmWrapper(struct tm time)
    : time_(time)
{
}
...
int
StructTmWrapper::minutes() const
{
    return time_.tm_min;
}


int StructTmWrapper::hour() const
{
...
int
StructTmWrapper::day_of_moth() const
{
...
Month
StructTmWrapper::month() const
{
    return IntToMonth(time_.tm_mon);
}


int StructTmWrapper::year() const
{
...
std::string
StructTmWrapper::ToString(const std::string& format) const
{
    std::vector<char> ret;
...
std::string
StructTmWrapper::DebugString() const
{
...
{
    const auto diff = TimetWrapper::Difference
    (
        TimetWrapper::FromGmt(StructTmWrapper(1970, Month::JANUARY ,1, 0, 0, 0)),
        dt
    );
...
TimetWrapper Int64ToDateTime(uint64_t i)
{
    ...
    (
        StructTmWrapper(1970, Month::JANUARY, 1, hours, acutal_minutes, actual_seconds)
    );
}
...
DateTime
DateTime::FromDate(int year, Month month, int day, TimeZone timezone)
{
...
DateTime
DateTime::FromDateTime(int year, Month month, int day, int hour, int minute, int second, TimeZone timezone)
{
...
DateTime
DateTime::CurrentTime(TimeZone timezone)
{
...
void
DateTime::set_month(Month month)
{
    auto s = AsStruct();
    s.set_month(month);
    UpdateTime(s);
...
TimetWrapper
ToTimetWrapper(const StructTmWrapper& s, TimeZone timezone)
{
    switch (timezone)
    {
    case TimeZone::GMT:
...
DateTime::DateTime(TimeZone timezone, const StructTmWrapper& time)
    : timezone_(timezone)
    , time_(ToTimetWrapper(time, timezone))
{
}
