...
namespace
{
    ...
    /// djb2 hash function
    u32 hash(const unsigned char* str, std::size_t length, u32 seed)
    {
        u32 hash = seed;
...
{
    return hash(reinterpret_cast<const unsigned char*>(str.data()), str.size(), last_id());
}
...
bool textfield(u32 id, std::string* val, render::DrawableFont* font, float size, const v2& pos, UiState& uistate, render::SpriteBatch* batch)
{
    ...
    if (uistate.kbd_item == id) {
        if (uistate.key.has_value()) {
            ...
            switch (k.key) {
            case SDLK_LEFT: text_state.onKeyLeft(shift != 0, val); changed = true; break;
            case SDLK_RIGHT: text_state.onKeyRight(shift != 0, val); changed = true; break;
            case SDLK_HOME: text_state.onKeyLineStart(shift != 0, val); changed = true; break;
            case SDLK_END: text_state.onKeyLineEnd(shift != 0, val); changed = true; break;
            case SDLK_BACKSPACE: text_state.onKeyBackspace(shift != 0, val); changed = true; break;
            case SDLK_DELETE: text_state.onKeyDelete(shift != 0, val); changed = true; break;
    ...
    if (uistate.kbd_item == id) {
        ...
        if (is_cursor_blink_visible != 0) {
            // Compute cursor x
            float cursor_x = rect.left;
            if (text_state.state.cursor > 0 && std::cmp_less_equal(text_state.state.cursor, val->size())) {
                const std::string substr = val->substr(0, text_state.state.cursor);
